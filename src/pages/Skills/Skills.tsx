import { DevIcon, DevIconSVG, TextUnderline } from "../../components/atoms";
const Skills = () => {
  return (
    <div id="skills" className=" bg-[#0a192f] text-gray-300">
      <div className="max-w-[1000px] mx-auto p-4 flex flex-col justify-center w-full h-full">
        <div>
          <TextUnderline text="Experience" />
          <p className="py-8">There are the technologies that I worked with.</p>
        </div>
        <div className="w-full grid grid-cols-3 sm:grid-cols-4 gap-4 text-center py-8">
          <DevIcon iconName="REACT" />
          <DevIcon iconName="REDUX" />
          <DevIcon iconName="ANGULAR" />
          <DevIcon iconName="JAVASCRIPT" />
          <DevIcon iconName="TYPESCRIPT" />
          <DevIcon iconName="NODEJS" />
          <DevIcon iconName="EXPRESS" />
          <DevIcon iconName="GRAPHQL" />
          <DevIcon iconName="MONGODB" />
          <DevIconSVG iconName="CIRCLE CI">
          ...
          <DevIcon iconName="AZURE" />
          <DevIcon iconName="TAILWIND" />
          <DevIcon iconName="CSS" />
          <DevIcon iconName="HTML" />
        </div>
      </div>
    </div>
  );
